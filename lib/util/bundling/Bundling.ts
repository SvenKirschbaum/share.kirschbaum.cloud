...
export class Bundling implements cdk.BundlingOptions {
    ...
    constructor(scope: IConstruct, private readonly props: BundlingProps) {
        ...
        if (this.relativeDepsLockFilePath.includes('..')) {
            throw new ValidationError(`ExpectedDepsLockFilePath` as any, `Expected depsLockFilePath: ${props.depsLockFilePath} to be under projectRoot: ${this.projectRoot} (${this.relativeDepsLockFilePath})`, scope);
        }
        ...
        if (props.preCompilation && !/\.tsx?$/.test(props.entry)) {
            throw new ValidationError(`PreCompilationTypescriptFiles` as any, 'preCompilation can only be used with typescript files', scope);
        }

        if (props.format === OutputFormat.ESM && !isEsmRuntime(props.runtime)) {
            throw new ValidationError(`ScriptModuleOutputFormatSupported` as any, `ECMAScript module output format is not supported by the ${props.runtime.name} runtime`, scope);
        }
    ...
    private createBundlingCommand(scope: IConstruct, options: BundlingCommandOptions): string {
        ...
        if (this.props.preCompilation) {
            ...
            if (!tsconfig) {
                throw new ValidationError(`CannotFindTsconfigJsonPre` as any, 'Cannot find a `tsconfig.json` but `preCompilation` is set to `true`, please specify it via `tsconfig`', scope);
            }
        ...
        if (this.props.sourceMap === false && this.props.sourceMapMode) {
            throw new ValidationError(`SourceMapModeCannotSource` as any, 'sourceMapMode cannot be used when sourceMap is false', scope);
        }
        ...
        if (this.props.nodeModules) {
            ...
            if (!pkgPath) {
                throw new ValidationError(`CannotFindPackageJsonProject` as any, 'Cannot find a `package.json` in this project. Using `nodeModules` requires a `package.json`.', scope);
            }
        ...
        return {
            tryBundle(outputDir: string) {
                ...
                if (!Bundling.esbuildInstallation.version.startsWith(`${ESBUILD_MAJOR_VERSION}.`)) {
                    throw new ValidationError(`ExpectedEsbuildVersion` as any, `Expected esbuild version ${ESBUILD_MAJOR_VERSION}.x but got ${Bundling.esbuildInstallation.version}`, scope);
                }
...
function toTarget(scope: IConstruct, runtime: Runtime): string {
    ...
    if (!match) {
        throw new ValidationError(`CannotExtractVersionRuntime` as any, 'Cannot extract version from runtime.', scope);
    }
